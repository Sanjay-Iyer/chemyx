...
from .. import config
from ..testing.mock_serial import MockChemyxSerial
    ...
    def connect(self):
        ...
        if not self.mock and self._serial_factory is None and not self.port:
            raise PumpConnectionError(
                "No Chemyx serial port configured. Set it in "
                "configs/machines/00_machine.local.yaml or CHEMYX_PORT."
            )
    ...
    def _friendly_error(self, exc):
        ...
        if (
            ...
            return (
                ...
                f"CHEMYX_PORT/machine YAML setting. "
                f"Original error: {exc}"
            )

...
(shipped in this repo at ``docs/reference/valve/titan_uart_protocol.pdf``; the same document
...
verbatim from the archived low-level bring-up script recorded in
``docs/ARCHIVE_MANIFEST.csv``, which was proven working on this hardware.
"""
...
from .. import config
from ..testing.mock_serial import MockMXValveSerial
...
class MX_valve:
    ...
    def connect(self) -> "MX_valve":
        if not self.mock and self._serial_factory is None and not self.address:
            raise ValveConnectionError(
                "No valve serial port configured. Set MXVALVE_PORT or the "
                "valve section in configs/machines/00_machine.local.yaml, "
        ...
        try:
            if self._serial_factory is not None:
                ...
                # Proven transport retained in the archive: set the
    ...
    def _friendly_error(self, exc) -> str:
        ...
        if "could not open" in text or "no such file" in text or "cannot find" in text:
            return (
                ...
                f"VCP driver, and MXVALVE_PORT/machine YAML setting. "

...
`chemyx_lab/workflows/first_real_chemyx_nmr.py`, and archived baseline evidence
...
| Operation | Command template | Parameters | Expected response | Physical state change | Used in workflow 01 |
| --- | --- | --- | --- | --- | --- |
...
| Start | `start` | None | Response is read and logged | Starts pump movement | Legacy helpers |
| Start with delay | `start 0` | Delay value preserved from proven workflow | Response is read and logged | Starts pump movement | Workflow 01 |
...
| Set delay | `set delay <value>` | Unresolved; command exposed by wrapper but not used in workflow 01 | Response is read | Unresolved | No |
| Set prime rate | `set primerate <value>` | Unresolved; command exposed by wrapper but not used in workflow 01 | Response is read | Unresolved | No |
...
| Operation | HTTP request | Parameters | Expected response | Physical state change | Used in workflow 01 |
| --- | --- | --- | --- | --- | --- |
...
| Read iFlow 1D settings | `GET /interfaces/iFlow/Settings/1D` | None | Settings mapping | No | Workflow 01 |

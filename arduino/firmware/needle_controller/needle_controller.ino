/*
  ...
  ELECTRICAL SAFETY:
  STEP_PIN, DIR_PIN, and ENABLE_PIN are logic signals intended for a verified
  open-collector/open-drain interface. Never wire UNO R4 GPIO directly to the
  DM542T PUL, DIR, or ENA inputs. The Arduino does not power the motor.

  MOTION_COMMISSIONED remains false for Arduino-only Test 1. Change it only
  after the interface, polarity, DM542T 5 V selector, current, microsteps,
  motor, fuse, and emergency disconnect have passed expert review.
*/

...
PowerShell examples (repo root, any Python with pyserial):
    ...
    python scripts\test_valve.py --mock                # no hardware needed
    python scripts\test_valve.py --mock --mock-level-logic   # demo the failure
    python scripts\test_valve.py                       # auto-detect FTDI port
    python scripts\test_valve.py --port COM7
    python scripts\test_valve.py --port COM7 --set-bcd # then POWER CYCLE
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "serial port (default: MXVALVE_PORT / configs/valve.local.json, "
...
def resolve_port(args) -> str:
    if args.port:
        return args.port
    cfg = config.load_valve_config()
    if cfg.port:
        print(f"Using configured port {cfg.port} "
              "(MXVALVE_PORT / configs/valve.local.json).")
        return cfg.port
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.mock and args.mock_level_logic:
        ...
        ports = args.ports if args.ports is not None else config.VALVE_POSITIONS
    ...
    if failures:
        ...
        print("If home worked but moves failed, check the command mode "
              "(Step 2) and see docs/valve_mx2_guide.md.")
        return 1

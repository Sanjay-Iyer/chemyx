...
from chemyx_lab.analysis.nmr import NmrProcessingError, analyze_dx_peak
from chemyx_lab.instruments.nmr import (
    NmrRpcClient,
    NmrRpcConfig,
    NmrRpcError,
...
def main() -> int:
    ...
    try:
        ...
        nmr_settings = config.load_nmr_settings(
            ...
            poll_seconds=(args.poll_seconds if args.poll_seconds is not None else machine.nmr.poll_seconds),
            max_wait_seconds=(args.max_wait if args.max_wait is not None else machine.nmr.max_wait_seconds),
            route=args.route,
    ...
    try:
        ...
        if nmr_settings.route == "iflow":
            one_d_template = (
                example_iflow_1d_settings()
                if args.mock_settings or args.dry_run
